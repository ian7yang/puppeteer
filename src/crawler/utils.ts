...
export function url2str(url:string, full:boolean = false) {
...
export const g = {
  BASE_DIR, LOG_DIR, ARCHIVES_DIR

};

const mouseEvents = new Set(['contextmenu']);

export function isMouseEvent(event: string) {
  if (!event) return false;
  if (event.startsWith('mouse') || event.startsWith('pointer') ||
      event.startsWith('touch')) return true;
  if (event.endsWith('click')) return true;
